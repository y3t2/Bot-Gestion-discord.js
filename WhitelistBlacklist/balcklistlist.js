...
module.exports = {
    name: 'blacklists',
    aliases: ['bls'],
    description: 'Liste les utilisateurs dans la blacklist.',
    async execute(message) {
        ...
        if (blacklist.length === 0) {
            return message.reply({
                embeds: [new EmbedBuilder()
                    .setColor(config.color)
                    .setDescription('Il n\'y a actuellement aucun utilisateur dans la blacklist.')
                    .setFooter({ text: config.footer })
                ]
            });
        }
        ...
        for (let i = 0; i < blacklist.length; i += chunkSize) {
            chunks.push(blacklist.slice(i, i + chunkSize));
        }

        let page = 0;

        const sendPage = () => {
            const blacklistList = chunks[page].map((user, index) => `${index + 1 + page * chunkSize}) <@${user.id}> (${user.id}) - Raison: ${user.reason}`).join('\n');
